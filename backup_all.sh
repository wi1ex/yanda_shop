...
# Регистрируем локальный alias shopminio на адресе 127.0.0.1:9000
./mc alias set shopminio http://127.0.0.1:9000 "${MINIO_ROOT_USER}" "${MINIO_ROOT_PASSWORD}" --api S3v4
# Создаём временную папку для mirror
TARGET="$MINIO_DIR/${MINIO_BUCKET}_${TIMESTAMP}"
mkdir -p "$TARGET"
# Делаем mirror бакета в локальную папку
./mc mirror --overwrite "shopminio/${MINIO_BUCKET}" "$TARGET"
